...
module.exports.plugin = function plugin (file, librarySettings, inputs) {
  ...
  if (inputs.bitrate_cutoff !== '') {
    ...
    if (currentBitrate <= inputs.bitrate_cutoff) {
      response.processFile = false;
      response.infoLog += `Current bitrate of ${currentBitrate} is below set cutoff of ${inputs.bitrate_cutoff}. Cancelling plugin. \n`;
      return response;
    }
  }
...
file_codec = file.ffProbeData.streams[0].codec_name;
  ...
  if (transcode == 1) {
    ...
    response.infoLog += `File is ${file_codec} with bitrate ${currentBitrate}. Transcoding to hevc!\n`;
  }
